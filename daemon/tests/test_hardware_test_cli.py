...
def test_hardware_test_cli_records_agent_direct_policy_review_without_enabling_direct_printing(
    tmp_path: Path,
) -> None:
    chain = _create_trusted_printer_chain(tmp_path)
    device_fingerprint = chain["device_fingerprint"]
    ...
    record_path = policy_output_dir / "agent-direct-policy-review.json"
    raw_stdout = stdout.getvalue()
    assert record_exit_code == 0
    assert exit_code == 0
    assert raw_stdout.strip() == "agent-direct-policy-review-recorded"
    assert "mock-minix-0194" not in raw_stdout
    assert device_fingerprint not in raw_stdout
    record = json.loads(record_path.read_text(encoding="utf-8"))
    assert record == {
...
def test_hardware_test_cli_inspects_agent_direct_policy_review_without_unlock(
    tmp_path: Path,
) -> None:
    chain = _create_trusted_printer_chain(tmp_path)
    device_fingerprint = chain["device_fingerprint"]
    ...
    policy_review_path = policy_output_dir / "agent-direct-policy-review.json"
    stdout = io.StringIO()

    exit_code = run(
        ["inspect-agent-direct-policy-review", str(policy_review_path)],
        stdout=stdout,
    )
    ...
    assert exit_code == 0
    assert raw_stdout.strip() == "agent-direct-policy-review-inspected"
    assert "mock-minix-0194" not in raw_stdout
    assert device_fingerprint not in raw_stdout
