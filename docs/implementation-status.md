...
- TDD red/green checks for the renderer Projects panel load/save/open/update/delete workflow against the daemon project client.
- Playwright MCP smoke against `http://127.0.0.1:5173/`: renderer loads after Projects open/update/delete UI changes, Projects panel renders Load/Save controls, and the browser console only shows the React DevTools hint.
...
1. Complete project management coverage: daemon-backed image asset import and migration away from single-document localStorage as the primary saved-project path.
